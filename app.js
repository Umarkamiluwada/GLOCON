      ...
      <div class="fb-comment ${isReply ? "fb-comment-reply" : ""}" data-comment-id="${c.id}">
        ...
        <div class="fb-comment-body">
          ...
          <div class="fb-comment-meta">
            <span class="fb-comment-time">${formatShortTime(c.created_at)}</span>
...
async function openPostDetail(postId) {
  ...
  const { data: post, error } = await sb
    ...
    .eq("id", postId)
    .single();
  if (error || !post) { container.innerHTML = `<div class="empty-state"><div class="icon">⚠️</div>${t("post_not_found")}</div>`; return; }
  const meta = await getPostMeta(post.id);
  container.innerHTML = "";
