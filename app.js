...
function screen(name) {
  ...
  if (name === "admin-dashboard") loadAdminTab(currentAdminTab);
  window.scrollTo(0, 0);
}
...
function renderPostCard(post, meta) {
  ...
  card.innerHTML = `
    <div class="post-head">
      ...
      <div class="who">
        <div class="name">${escapeHtml(name)}${verifiedBadge(prof)}${isBoosted ? ' <span class="boosted-tag">🚀 ' + t("boosted") + '</span>' : ""}</div>
    ...
    <div class="post-actions">
      ...
      ${isMine && !isBoosted ? `<button class="action-btn" data-action="boost"><span class="ic">🚀</span></button>` : ""}
    </div>
...
function verifiedBadge(prof) {
  return prof && prof.is_verified ? ' <span class="verified-badge" title="Verified">✔️</span>' : "";
}
...
async function loadComments(postId, container) {
  ...
  const { data, error } = await sb
    .from("comments")
    .select("id, content, created_at, profiles!comments_user_id_fkey ( first_name, last_name )")
    .eq("post_id", postId)
...
async function loadReelComments(reelId, container) {
  ...
  const { data, error } = await sb
    .from("reel_comments")
    .select("id, content, created_at, profiles!reel_comments_user_id_fkey ( first_name, last_name )")
    .eq("reel_id", reelId)
    .order("created_at", { ascending: true });
...
async function loadWallet() {
  ...
  const { data: txs, error } = await sb
    ...
    .limit(50);
  if (error) { list.innerHTML = `<div class="empty-state"><div class="icon">⚠️</div>${error.message}</div>`; return; }
  if (!txs || txs.length === 0) { list.innerHTML = `<div class="empty-state"><div class="icon">💰</div>${t("no_transactions_yet")}</div>`; return; }
...
$("#wallet-verify-btn").addEventListener("click", async () => {
  showLoading("posting");
  const { error } = await sb.rpc("buy_verify_badge");
  hideLoading();
  if (error) { showToast(error.message); return; }
  showToast(t("posted_success"));
  loadWallet();
...
async function loadAdminTab(tab) {
  ...
  content.innerHTML = `<div class="empty-state"><div class="icon">⏳</div>${t("loading")}</div>`;
...
async function renderAdminOverview() {
  ...
  if (error) { content.innerHTML = `<div class="empty-state"><div class="icon">⚠️</div>${error.message}</div>`; return; }
  content.innerHTML = `
...
async function renderAdminUsers(search) {
  const content = $("#admin-content");
  const { data, error } = await sb.rpc("admin_list_users", { p_search: search || null });
  if (error) { content.innerHTML = `<div class="empty-state"><div class="icon">⚠️</div>${error.message}</div>`; return; }
  ...
  rows.innerHTML = (data || []).map((u) => {
    ...
    return `
      <div class="admin-row">
        <div class="admin-row-title">${escapeHtml(name)} @${escapeHtml(u.username || "")} ${u.is_verified ? "✔️" : ""} ${u.is_suspended ? "🚫" : ""}</div>
    ...
    `;
  }).join("") || `<div class="empty-state"><div class="icon">👥</div>${t("no_users_found")}</div>`;
...
async function renderAdminPurchases() {
  ...
  if (pending.length === 0) { content.innerHTML = `<div class="empty-state"><div class="icon">✅</div>No pending buy requests.</div>`; return; }
...
async function renderAdminCashouts() {
  ...
  if (error) { content.innerHTML = `<div class="empty-state"><div class="icon">⚠️</div>${error.message}</div>`; return; }
  const pending = (data || []).filter((r) => r.status === "pending");
  if (pending.length === 0) { content.innerHTML = `<div class="empty-state"><div class="icon">✅</div>No pending cash-out requests.</div>`; return; }
  content.innerHTML = pending.map((r) => `
    <div class="admin-row">
      <div class="admin-row-title">${r.coins} GLOCOIN → ₦${r.naira_amount}</div>
...
async function renderAdminReports() {
  ...
  if (open.length === 0) { content.innerHTML = `<div class="empty-state"><div class="icon">✅</div>No open reports.</div>`; return; }
...
async function renderAdminPosts() {
  ...
  `).join("") || `<div class="empty-state"><div class="icon">📝</div>No posts.</div>`;
